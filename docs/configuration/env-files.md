...
- Runtime ключи должны быть задокументированы и перечислены в [`services/backend/.env.example`](services/backend/.env.example:1).
...
- Локальные скрипты должны загружать [`.env`](.env:1) и [`.env.oauth`](.env.oauth:1) **до** импорта [`services/backend/llm_agent_platform/config.py`](services/backend/llm_agent_platform/config.py:1).
...
3) импортировать [`services/backend/llm_agent_platform/config.py`](services/backend/llm_agent_platform/config.py:1) и использовать значения из `os.environ`
...
- [`services/backend/scripts/get_openai-chatgpt_credentials.py`](services/backend/scripts/get_openai-chatgpt_credentials.py:1)
...
- если ключ используется в коде (см. [`services/backend/llm_agent_platform/config.py`](services/backend/llm_agent_platform/config.py:1) и прямые `os.environ.get` вызовы в runtime), он должен быть перечислен в `environment:` compose-файла.
...
- ключи в [`services/backend/llm_agent_platform/config.py`](services/backend/llm_agent_platform/config.py:1)
...
Bootstrap script [`services/backend/scripts/get_openai-chatgpt_credentials.py`](services/backend/scripts/get_openai-chatgpt_credentials.py:1) использует локальные OAuth env:
...
- неиспользуемые ключи удалены из [`services/backend/.env.example`](services/backend/.env.example:1),
