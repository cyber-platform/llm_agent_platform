...
Приоритет анализа — streaming path. Non-stream тоже покрыт, но рассматривается как secondary parity layer. Причина простая: именно stream contract определяет совместимость с KiloCode runtime/UI и именно stream reasoning уже зафиксирован в нашем каноне как обязательный public behavior.
...
5. Передает ли наша платформа reasoning клиенту и в каком формате, особенно в stream path.
6. Что теряется относительно reference KiloCode в stream и non-stream.
...
- Для нашей задачи самый важный факт: streaming reasoning в платформе действительно передается клиенту и нормализуется в KiloCode-compatible `delta.reasoning_text`.
...
## Stream-first takeaway

Если следующему агенту нужно быстро восстановить главное без чтения всего документа, достаточно зафиксировать пять тезисов:

1. KiloCode как клиент ожидает reasoning в OpenAI-compatible stream через `choices[0].delta.reasoning_text`, см. [`externel_projects/kilocode/packages/opencode/src/provider/sdk/copilot/chat/openai-compatible-chat-language-model.ts:459`](externel_projects/kilocode/packages/opencode/src/provider/sdk/copilot/chat/openai-compatible-chat-language-model.ts:459).
2. Наша платформа именно это и делает: private backend events `response.reasoning*.delta` и `response.reasoning_summary*.delta` превращаются в SSE chunk с `reasoning_text`, см. [`services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:333`](services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:333) и [`services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:344`](services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:344).
3. Этот stream contract уже закреплен в docs и tests, см. [`docs/providers/openai-chatgpt.md:147`](docs/providers/openai-chatgpt.md:147), [`docs/testing/suites/openai-contract.md:31`](docs/testing/suites/openai-contract.md:31), [`services/backend/llm_agent_platform/tests/test_openai_chatgpt_runtime.py:1184`](services/backend/llm_agent_platform/tests/test_openai_chatgpt_runtime.py:1184).
4. Stream path у нас сейчас сильнее non-stream path: он понимает несколько reasoning delta event types и имеет fallback extraction из `response.output_item.done`, см. [`services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:391`](services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:391).
5. Главный remaining gap в stream — не отсутствие reasoning как такового, а потеря full-fidelity semantics: encrypted metadata, `output_index` correlation и multi-part reasoning lifecycle, которые reference KiloCode хранит internally, см. [`externel_projects/kilocode/packages/opencode/src/provider/sdk/copilot/responses/openai-responses-language-model.ts:818`](externel_projects/kilocode/packages/opencode/src/provider/sdk/copilot/responses/openai-responses-language-model.ts:818).
...
Это главный runtime path для parity c KiloCode. Если ориентироваться на пользовательский runtime effect, то именно этот раздел важнее non-stream parsing.
...
- если вопрос стоит как `передает ли платформа reasoning в реальном рабочем интерактивном режиме`, ответ — да;
- именно streaming path сейчас является strongest evidence of parity;
- текущий риск для stream не в потере самого reasoning текста, а в потере его richer structure.
...
- Stream path additionally covers fallback extraction from final items, если delta не пришла.
- Usage path forwards `reasoning_tokens`.
- Contract закреплен тестами и docs.

### What matters most for the current question

- Если consumer — KiloCode или другой OpenAI-compatible streaming client, reasoning из нашей платформы уже читается.
- Наиболее важный подтвержденный surface — SSE `choices[0].delta.reasoning_text`.
- Следовательно, для stream-centric use case платформа reasoning forwards успешно.
...
### 1. Preserve stream contract as the primary invariant

Любые дальнейшие изменения не должны ломать уже существующий public stream contract:

- `response.reasoning*.delta` -> `choices[0].delta.reasoning_text`
- fallback from final output items should remain
- `usage.completion_tokens_details.reasoning_tokens` should remain

Primary guard references:

- [`services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:333`](services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:333)
- [`services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:344`](services/backend/llm_agent_platform/api/openai/providers/openai_chatgpt.py:344)
- [`services/backend/llm_agent_platform/tests/test_openai_chatgpt_runtime.py:1184`](services/backend/llm_agent_platform/tests/test_openai_chatgpt_runtime.py:1184)

### 2. Fix non-stream parser parity next
...
### 3. Decide whether encrypted reasoning metadata belongs in public contract
...
### 4. Keep `reasoning_text` as compatibility baseline
...
- Для stream-centric сценария, который для нас важнее всего, reasoning forwarding уже работает и подтвержден и кодом, и тестом, и docs canon.
