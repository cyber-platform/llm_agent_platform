...
| Runtime services | Даёт общие runtime ports: monitoring runtime store, admin monitoring read-model/materialization, monitoring adapters, state-path resolution и quota transport helpers. | Shared infrastructure and core | materialized in code |
...
6. `Quota and account state runtime` и `Runtime services` обновляют routing state, monitoring runtime store и persistence artifacts, когда это требуется сценарием.
