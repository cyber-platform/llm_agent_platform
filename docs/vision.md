...
Текущий Source of Truth находится в [`docs/`](docs:1). Временные материалы из [`plans/`](plans:1) и [`tasks_descriptions/`](tasks_descriptions:1) могут использоваться как исторический контекст, но не должны быть обязательными для понимания актуальной архитектуры.
