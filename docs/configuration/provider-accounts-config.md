...
- Runtime router: [`llm_agent_platform/services/account_router.py`](llm_agent_platform/services/account_router.py:114)
- Route/provider resolution: [`llm_agent_platform/api/openai/pipeline.py`](llm_agent_platform/api/openai/pipeline.py:97)
...
- platform/runtime state — отдельные mutable state файлы в [`STATE_DIR`](llm_agent_platform/config.py:30);
- provider-specific monitoring snapshots — тоже отдельные mutable state файлы в [`STATE_DIR`](llm_agent_platform/config.py:30).
...
- runtime не выполняет BY-N rotation при `<= 0` в [`QuotaAccountRouter.register_success()`](llm_agent_platform/services/account_router.py:232)
...
- см. [`QuotaAccountRouter._quota_reset_timestamp()`](llm_agent_platform/services/account_router.py:582)
...
- если `groups` заданы, default group определяется runtime как первая группа в конфиге — см. [`_resolve_group_models()`](llm_agent_platform/api/openai/pipeline.py:111)
