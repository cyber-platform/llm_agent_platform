...
Связанные решения и спецификации:
...
- [`.env.oauth`](tasks_descriptions/tasks/019-env-split-and-explicit-container-env.md:1) — storage значений bootstrap (создается пользователем локально, не коммитится).
...
- Bootstrap ключи должны быть задокументированы в [`.env.oauth.example`](tasks_descriptions/tasks/019-env-split-and-explicit-container-env.md:1).
...
2) загрузить [`.env.oauth`](tasks_descriptions/tasks/019-env-split-and-explicit-container-env.md:1) с override (bootstrap перекрывает runtime при необходимости)
...
- политика зафиксирована в [`Spec 019`](plans/019-env-separation-runtime-vs-oauth-bootstrap-spec.md:1), Appendix A.
