...
Этот документ закрывает замечание `F-005` из [`plans/critic-openai-chatgpt-monitoring-and-usage-plans-2026-03-30.md`](plans/critic-openai-chatgpt-monitoring-and-usage-plans-2026-03-30.md:89).

Critic справедливо отмечает, что в [`plans/openai-chatgpt-monitoring-frontend-prototype.md`](plans/openai-chatgpt-monitoring-frontend-prototype.md:1) пилотный UI объявлен как `без авторизации`, а после решения [`F-004`](plans/discussion-f-004-activate-semantics-and-routing-boundary-openai-chatgpt-monitoring.md:1) в пилоте остаётся mutating action `Activate`, пусть и только как in-memory override.

Это допустимо только как **временное локальное PoC-допущение**, а не как baseline для shared dev, team-facing или production-like deployment.
...
- какой trigger возвращает вопрос в Stage 2 для freeze auth and RBAC boundary.
...
В текущем UI-плане зафиксировано:

- без авторизации;
- без ролей и разграничений;
- только админский monitoring UI;
...
- проект живёт в локальном dev-контуре, см. [`docs/run/dev.md`](docs/run/dev.md:1);
- runtime env и deployment boundary описаны в [`docs/configuration/env-files.md`](docs/configuration/env-files.md:1);
...
Проблема не в том, что no-auth для PoC невозможен. Проблема в том, что без явной фиксации границы решение начинает выглядеть как канонический baseline.
...
Для `F-005` фиксируется простое правило:
...
- требует явного trigger-а на возврат в Stage 2.
...
После `F-004` action `Activate` остаётся в пилоте, но это не отменяет нужду в boundary.
...
Блокирующих open questions для `F-005` больше не осталось.

Остаются только downstream steps, которые не меняют freeze решения:

1. переписать baseline UI plan под accepted security boundary;
2. вынести future auth/RBAC hardening в отдельный backlog and Stage 2 track;
3. синхронизировать wording в downstream docs, когда pilot выйдет за пределы local PoC.
