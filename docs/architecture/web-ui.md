...
Для текущего materialized operator slice уже реализован quick auth baseline:

- login screen использует `services/user_service` как identity boundary;
- frontend хранит access token в локальном auth state без refresh-token flow;
- protected admin shell открывается только после успешного login;
- backend принудительно проверяет доступ к `/admin/*` независимо от UI state.
...
- `Frontend service` использует `services/user_service` для login flow и `Backend service` для admin API;
...
- Текущая реализация также покрывает login screen, protected admin shell, logout flow и auto-logout на predictable admin auth errors.
- Для [`openai-chatgpt`](../providers/openai-chatgpt.md:1) текущий local-only frontend также materialize-ит admin-facing policy editor для key-scoped `reasoning_effort` overrides поверх backend admin API.
- Policy editor работает в current key/group context, поддерживает explicit pass-through state, `force` и `default_if_absent`, а также показывает backend-derived capability constraints и read-only raw capability JSON drawer.
